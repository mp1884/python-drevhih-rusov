...
_PyRun_InteractiveLoopObject(FILE *fp, PyObject *filename, PyCompilerFlags *flags)
{
    ...
    int err_cnt = 0;
    char *quotes[] = {
            "Нет, сказки это все. Давайте начнём с чего попроще",
            "Если вы программист то вы должны значть счёт древних русов",
            "Ну вы же все здравомыслящие люди, почему вы не видите что вас зомбируют",
            "А никакого питона в природе нет",
            "А я знаю счёт древних шизов",
            "А нам навязывают ложную математику, чтобы обманывать и ограблять русов",
            "Мы шизы! Это наш интерпретатор! Здесь будут наши порядки! Кому не нравится - чемодан, вокзал, нахуй!",
    };
    do {
