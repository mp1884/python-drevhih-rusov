...
pymain_header(const PyConfig *config)
{
    ...
    fprintf(stderr, "Питон тот, который сегодня, в природе не работает.\n");
    fprintf(stderr, "Поэтому я сделал свой интерпретатор, называется \"дам всем в рот\".\n");
    fprintf(stderr, "Вот он, интерпретатор:\n\n");
